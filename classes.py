...
class xxxScreen:
    def __init__(self, title: str, resolution: tuple[int,int]):
        sdl_ext.init()
        self.window = sdl_ext.Window(title, resolution)
        self.sprite_factory = sdl_ext.SpriteFactory(sdl_ext.SOFTWARE)
        self.sprite_renderer = self.sprite_factory.create_sprite_render_system(self.window)
        

    def Show(self):
        self.window.show()

    def DrawSprite(self, sprite: sdl_ext.TextureSprite | sdl_ext.SoftwareSprite, position: tuple[int,int]):
        ...
        for row in buffer:
            for col in row:
                ...
                if pixel == 1: #if pixel on
                    ...
                    pixel_colour = (0,0,0)
                pygame.draw.rect(self.screen, pixel_colour, [col_counter, row_counter, 1*self.scale, 1*self.scale])
                #for x in range(0, self.scale):
                #    for y in range(0,self.scale):
                #        pygame.draw.rect(self.screen, pixel_colour, [row_counter+x, col_counter+y, 1*self.scale, 1*self.scale])
                        
                col_counter += self.scale
            col_counter = 0
            row_counter += self.scale
